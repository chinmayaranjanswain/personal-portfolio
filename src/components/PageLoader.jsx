...
export default function PageLoader({ onComplete }) {
  ...
  // Counter animation — faster on mobile
  useEffect(() => {
    if (counter >= 100 || phase !== 'loading') return;
    let speed;
    if (isMobile) {
      speed = counter < 30 ? 30 : counter < 70 ? 20 : counter < 95 ? 35 : 60;
    } else {
      speed = counter < 20 ? 65
        : counter < 50 ? 50
          : counter < 75 ? 70
  ...
  }, [counter, phase, isMobile]);

  // Initialize spiral animation (Desktop only)
  useEffect(() => {
    if (isMobile) return;
  ...
  useEffect(() => {
    ...
    tl.to({}, { duration: isMobile ? 0.1 : 0.3 });
  ...
  }, [counter, phase, onComplete, isMobile]);

  if (phase === 'done') return null;

  // ---- MOBILE LOADER: minimal + lightweight ----
  if (isMobile) {
    return (
      <div
      ...
      >
        <div className="intro-overlay-top" style={{ position: 'relative', top: 'auto', textAlign: 'center' }}>
        ...
        <div className="intro-overlay-bottom" style={{ position: 'relative', bottom: 'auto' }}>
          <div className="intro-overlay-counter">
            <span className="intro-overlay-num" style={{ fontSize: '1.5rem' }}>{counter}</span>
            <span className="intro-overlay-pct" style={{ fontSize: '0.8rem' }}>%</span>
          </div>
        </div>
      </div>
    );
  ...
  // ---- DESKTOP LOADER: full spiral canvas ----
  return (
    <div
      className="intro-sequence"
      ref={containerRef}
